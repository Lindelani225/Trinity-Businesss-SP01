...
namespace BusinesssTrinitySP01.Controllers
{
    public class ColorsController : Controller
    {
        ...
        {
            var q = from temp in db.colors where temp.BoxId == Id select temp.Image;
            byte[] cover = q.First();
            return cover;
        }
        ...
        public ActionResult Create(Color color)
        {
            ...
            {
                HttpPostedFileBase file = Request.Files["ColorImage"];
                UploadImage service = new UploadImage();
                color.Image = service.ConvertToBytes(file);
                db.colors.Add(color);
                db.SaveChanges();
